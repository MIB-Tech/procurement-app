...
export const ModelCellSkeleton: FC<{
  iconSize?: number;
  titleHeight?: number;
  subTitleHeight?: number;
}> = ({ iconSize = 30, titleHeight = 10, subTitleHeight = 20 }) => (
...
export const ModelCell = <M extends ModelEnum>({
  ...
  return (
    <div className='d-flex align-items-center'>
      ...
      <div>
        {hydraSubtitle ? (
          <div>{hydraSubtitle(item)}</div>
        ) : (
          <div className='text-muted fs-9 -text-truncate'>{subTitle}</div>
        )}
        ...
        {readOnly ? (
          ...
          <ModelLink
            item={item}
            className='text-gray-800 -text-truncate'
          />
        )}
      </div>
    </div>
