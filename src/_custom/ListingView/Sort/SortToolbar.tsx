...
const SortToolbar = <M extends ModelEnum>({onChange, columnDef, sort = {}, className}: Props<M>) => {
        ...
        return (
          <>
            <ClickAwayListener
              ...
              <Dropdown show={open} className={className}>
                <Dropdown.Toggle
                  as={DivToggle}
                  onClick={() => {
                    setOpen(true)
                ...
                <Dropdown.Menu className="px-3 py-3 w-300px">
                  <div className="d-flex flex-column gap-2">
                    {columnNames.map((columnName) => {
                      ...
                      const {title = stringToI18nMessageKey(name)} = columnDef[columnName]
