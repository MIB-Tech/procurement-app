...
export const StringField: FC<StringFieldProps> = ({
  ...
  const { format } = column;
  const icon: boolean | undefined =
    !hideIcon &&
    !(
      format &&
      [StringFormat.Password, StringFormat.Icon, StringFormat.Select].includes(
        format
      )
    );
  ...
  switch (format) {
    ...
    case StringFormat.Icon:
      children = (
        <AutocompleteBase
          options={ICONS.map(({ path }) => path).filter(
          ...
          getOptionLabel={(path) =>
            ICONS.find((icon) => icon.path === path)?.name || path
          }
                  ...
                  label={
                    <div className='d-flex align-items-center'>
                      ...
                      <div className='text-truncate'>
                        {ICONS.find(({ path }) => path === option)?.name ||
                          option}
                      </div>
                    </div>
                  }
    ...
    case StringFormat.Select:
      ...
      const getOptionVariant: RadioProps<string>["getOptionVariant"] = (
        option
      ) => {
