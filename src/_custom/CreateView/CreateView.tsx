...
export const CreateView = <M extends ModelEnum>({ modelName }: CreateViewProps<M>) => {
  const { views } = useMapping<M>({ modelName });
  const { state } = uselocation();
  const view = (views?.find(view => view.type === ViewEnum.Create) || DEFAULT_CREATE_VIEW) as CreateViewType<M>;
