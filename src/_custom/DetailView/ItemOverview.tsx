...
export const ItemOverview = <M extends ModelEnum>({
  modelName,
  children,
}: {
  modelName: M;
  ...
  const _operations = useMemo(() => {
    ...
    return (
      itemOperationRoutes?.({ item, operations: itemOperations }) ||
      itemOperations
    );
  ...
  return (
    <div className='card card-bordered mb-3'>
      <div className={clsx("card-body", children && "pb-0")}>
        <div
          className={clsx(
            "d-flex flex-wrap flex-xs-nowrap",
            children && "mb-3"
          )}
        >
          {isLoading && (
            ...
            <div className='d-flex justify-content-between align-items-start flex-wrap gap-3'>
              <div className='d-flex flex-column'>
                <div className='d-flex flex-wrap fw-semibold fs-5 text-gray-500 '>
                  {item && (
                    <Trans id={stringToI18nMessageKey(item?.["@type"])} />
                  )}
                </div>
                <div className='d-flex align-items-center'>
                  <a
                    href='#'
                    className='text-gray-800 text-hover-primary fs-2 fw-bold mt-1 me-3'
                  >
                    {item?.["@title"]}
                  </a>
                </div>
              ...
              <div className='d-flex gap-3'>
                {item &&
                  customActions?.map(({ render }, index) => (
