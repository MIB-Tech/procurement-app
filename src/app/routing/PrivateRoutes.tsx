...
export function PrivateRoutes() {
    ...
    const defaultOperation = operations
        ...
        .find(
            (operation) =>
                operation.isMenuItem &&
                operation.operationType === ViewEnum.Listing
        );
    ...
    return (
        <RoutesWithPasswordCheck>
            ...
            >
                {CUSTOM_ROUTES.filter((route) => isGranted(route.granted)).map(
                    (route) => {
                        return (
                            <Route
                                key={route.path}
                ...
                {operations
                    .filter((operation) =>
                        Object.values(ModelEnum).includes(
                    ...
                    .map((operation) => {
                        ...
                        const path =
                            getRoutePrefix(resourceName) + "/" + suffix;
                        let element: ReactNode;
                        switch (operationType) {
                            case ViewEnum.Listing:
                                element = (
                                    <ListingView modelName={resourceName} />
                                );
                                break;
                            case ViewEnum.Create:
                                element = (
                                    <CreateView modelName={resourceName} />
                                );
                                break;
                            case ViewEnum.Detail:
                                element = (
                                    <DetailView modelName={resourceName} />
                                );
                                break;
                            case ViewEnum.Update:
                                element = (
                                    <UpdateView modelName={resourceName} />
                                );
                                break;
                            case ViewEnum.Delete:
                                element = (
                                    <DeleteView modelName={resourceName} />
                                );
                                break;
                        }
                        ...
                        return (
                            ...
                            >
                                {(
                                    Object.keys(
                                        detailView.columns || columnDef
                                    ) as Array<keyof Model<any> | string>
                                ).map((embeddedColumnName) => {
                                    const _embeddedColumnName =
                                        embeddedColumnName.toString();

                                    return (
                                        <Route
                                            key={_embeddedColumnName}
                                            path={camelCaseToDash(
                                                _embeddedColumnName
                                            )}
                                            element={
                                                <>`${_embeddedColumnName}`</>
            ...
            <Route
                ...
                element={
                    <Navigate
                        to={`/error/${operations.length === 0 ? 403 : 404}`}
                    />
                }
            />
        </RoutesWithPasswordCheck>
