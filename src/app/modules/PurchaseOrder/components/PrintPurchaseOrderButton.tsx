...
> = ({ ...props }) => {
  ...
  const params = useMemo<PurchaseOrderPrint | undefined>(() => {
    ...
    return {
      ...item,
      referentFullNames: referents
        .map((referent) => (referent as HydraItem)["@title"])
        .join(", "),
      buyer: (buyer as HydraItem | undefined)?.["@title"],
      reference: ref,
      taxType: taxIncluded ? "TTC" : "HT",
      currencyCode: unit,
      stimulsoftCurrencyCode: unit === "DH" ? "DHS" : unit,
      ...
      lines: purchaseOrderProducts.reduce((lines, purchaseOrderProduct) => {
        ...
        return [
          ...lines,
          {
            ...
            netPrice: getNumberUnit({
              value: taxIncluded ? priceInclTax : priceExclTax,
              precision,
              unit: false,
            }),
            discountedUnitPrice: getNumberUnit({
