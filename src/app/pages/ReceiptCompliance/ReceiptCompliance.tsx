...
import {
  ...
  ComplianceStatus,
} from "../../modules/ReceiptProduct/Model";
...
export const ReceiptCompliancePage: FC = () => {
  ...
  const mutation = useMutation<
    ...
    AxiosError<string>,
    {
      receiptProducts: Array<
        Pick<
          ReceiptProductModel,
          "id" | "complianceStatus" | "complianceReserve"
        >
      >;
    }
  ...
  const submitformik = useFormik({
    ...
    onSubmit: (values) => {
      mutation.mutate({
        ...values,
        receiptProducts: values.receiptProducts.map(
          ({ id, complianceStatus, complianceReserve }) => ({
            id,
            complianceStatus,
            complianceReserve,
                    ...
                    <table className='table table-sm  gy-1 align-middle mb-0'>
                      <thead className='fs-7 text-gray-400 text-uppercase border-bottom text-nowrap'>
                        <tr>
                          ...
                          <th>
                            <Trans id='RECEIVED_QUANTITY' />
                          </th>
                          <th>
                            <Trans id='COMPLIANCE' />
                          </th>
                          <th>
                            <Trans id='COMPLIANCE_RESERVE' />
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {submitformik.values.receiptProducts.map(
                          (item, index, receiptProducts) => (
                            <tr
                              key={item.id}
                              className={clsx(
                                index < receiptProducts.length - 1 &&
                                  "border-bottom"
                              )}
                            >
                              <td>
                              ...
                              <td>
                                {item.complianceStatus ===
                                  ComplianceStatus.ConformWithReserve && (
                                  <InputField
