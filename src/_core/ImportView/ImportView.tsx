...
const getWorkSheet = (workbook: WorkBook) =>
  workbook.Sheets[workbook.SheetNames[0]];
export const getData = <M extends ModelEnum>({
  workbook,
  mapping,
}: Required<Value<M>>) => {
  const workSheet = getWorkSheet(workbook);
  const sheetData = utils.sheet_to_json<Record<string, any>>(workSheet, {
    raw: false,
  });
  ...
  return sheetData.map((row) => {
    ...
    for (const currentValue in row) {
      const columnName = (Object.keys(mapping) as Array<keyof Model<M>>).find(
        (key) => mapping[key] === currentValue
      );
...
export const ImportView = <M extends ModelEnum>({
  modelName,
  view,
}: ImportViewProps<M>) => {
  ...
  const columnNames = fields
    ? (Object.keys(fields) as Array<keyof Model<M>>).filter((columnName) => {
                  ...
                  {columnNames.map((columnName) => {
                    ...
                    return (
                      <th key={columnName.toString()}>
                        <div
                          className={clsx(
                            !columnMapping.nullable && "required"
                          )}
                        >
                          <TitleContent
