...
import { useTenant } from "../../app/modules/utils";
...
export const useAuth = () => {
  ...
  const { user } = useSelector<RootState>(({ auth }) => {
    return auth;
  }, shallowEqual) as Required<Pick<AuthState, "user">> &
    Pick<AuthState, "tenant">;
...
export const GrantedLink: FC<GrantedProps> = ({
  ...
  let path =
    isGranted([{ resourceName, operationType }]) &&
    (to || getPath({ suffix, resourceName }));
  if (!path) {
  ...
  return (
    <Link
      to={to || generatePath(path, params)}
      {...props}
    >
      {children}
    </Link>
